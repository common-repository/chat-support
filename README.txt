=== ChatSupport ===
Contributors: Cotrox, GraficaRuleZ
Tags: Chat, Support, WhatsApp, Supporto, Contact
Donate link: https://www.paypal.com/paypalme/graficarulez
Version: 1.0.5
Stable Tag: 1.0.5
Requires at least: 6.1
Tested up to: 6.4
Requires PHP: 7.2
License: GPL v2 or Later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

Add a new and Fluent WhatsApp Chat Support Button to your WordPress Website.

== Description ==
Add a new and Fluent WhatsApp Chat Support Button to your WordPress Website.

== Installation ==
Find our Plugin on WordPress and just press the Install button to let WordPress install it for you on your Website. Then press 'Activate' to switch it on.

To let WhatsSupport redirect users to the right link just go on your Admin Dashboard > WASupport and put in Settings your Business Phone Number.

== Changelog ==
= 1.0.5 =
* Icon FA Reference Fix
= 1.0.4 =
* Icon Center Fixes
= 1.0.3 =
* Default or Choosed Color Introduction
= 1.0.2 =
* Color Introduction
= 1.0.1 =
* Problems Fixes
= 1.0.0 =
* First Plugin Version Release

== Upgrade Notice ==
Upgrade the plugin to be always up to date.

== Links ==
Public: https://wordpress.org/plugins/chat-support/
SVN-Repository: https://plugin.svn.wordpress.org/plugins/chat-support/